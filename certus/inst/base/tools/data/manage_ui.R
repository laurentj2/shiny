  ...
  list(
    wellPanel(
      radioButtons(inputId = "dataType", label = "Load data:",
                   c("Load yahoo data" = "examples", "state" = "state"),
...
observe({
  ...
  isolate({
    ...
    tickers = spl('MSFT,AAPL,GOOGL,NVDA,QQQ,SPY,GS,C,MVIS')

    data <- new.env()
    getSymbols(tickers, src = 'yahoo', from = '2007-01-01', env = data, auto.assign = T)
    for(i in ls(data)) data[[i]] = adjustOHLC(data[[i]], use.Adjusted=T)                            
    
    bt.prep(data, align='keep.all', dates='2008::2009') 
    
    prices = data$prices
    prices = ROC(prices)

    r_data[['diamonds']] <- as.data.frame(prices)
