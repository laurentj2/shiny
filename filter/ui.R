...
shinyUI(pageWithSidebar(
	...
	sidebarPanel(   
		...
		#),
		selectInput("oos1", strong("Training set begin bar"), choices =  seq(1,3000,by=25),selected=0),
		selectInput("oos", strong("Training set size"), choices =  seq(100,3000,by=25),selected=625),
		br(),
		selectInput("smaLen", strong("Filter length:"), choices =  seq(20,300,by=10),selected=150),
		selectInput("ub", strong("Low-pass filter setting:"), choices =  seq(1,15,by=1),selected=3),
		#selectInput("up", strong("Upperband:"), choices =  seq(0,10,by=1),selected=10),
		...
		br(),
		selectInput("overfit", strong("Overfitting protection:"), choices =  seq(0,6,by=1),selected=3),
		br(),
		br(),
		submitButton("Run"),
		htmlOutput("status")
